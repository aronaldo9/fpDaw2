...
class CancionRepository
{
    ...
    static function addCancion($datos, $files)
    {
        ...
        $q = "INSERT INTO cancion VALUES (null,'" . $datos['titulo'] . "','" . $datos['autor'] . "'," . $datos['duracion'] . ",'" . $files['img']['name'] . "','" . $files['mp3']['name'] . "','" . $_SESSION['user']->getId() . "')";
        var_dump($q);
        $result = $bd->query($q);
    }
    ...
    {
        $bd = Conectar::conexion();
        $result = $bd->query("SELECT * FROM cancion");
